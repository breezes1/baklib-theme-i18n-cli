...
export default async function extractKeys() {
  ...
  // 合并并写入 mainLanguage
  const mainNormal = mergeLocaleObj(mainNormalOrigin, mainNormalObj);
  const mainSchema = mergeLocaleObj(mainSchemaOrigin, mainSchemaObj);
  ...
  for (const lang of langs.filter(l => l !== mainLang)) {
    ...
    const mergedNormalLang = mergeLocaleObj(normalOrigin, mainNormal, true);
    const mergedSchemaLang = mergeLocaleObj(schemaOrigin, mainSchema, true);
